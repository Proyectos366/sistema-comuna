/**
  ...
  maneja la lógica para actualizar el estado de un curso a verificado a través de una solicitud
  PATCH. Utiliza Prisma para la interacción con la base de datos, un servicio de validación
  para asegurar la validez de los datos, y un sistema de registro de eventos para la auditoría.
  @module
*/
...
/**
  Maneja las solicitudes HTTP PATCH para verificar un curso.
  @async@function PATCH
  @param {Request} request - Objeto de la solicitud que contiene el ID del curso y del vocero a verificar.
  ...
  try {
    ...
    const verificarCurso = await prisma.curso.update({
      ...
      data: {
        ...
        updatedAt: new Date(),
        id_verifico: validaciones.id_usuario,
      },
    });

    // 5. Consulta el curso verificado junto con información relevante
    const participanteCursoFormacionVerificado = await prisma.curso.findFirst({
      where: { borrado: false, id: verificarCurso.id },
      include: {
        voceros: {
          select: {
            id: true,
    ...
    if (!participanteCursoFormacionVerificado) {
      await registrarEventoSeguro(request, {
        tabla: "curso",
        ...
        datosAntes: null,
        datosDespues: participanteCursoFormacionVerificado,
      });
    ...
    await registrarEventoSeguro(request, {
      tabla: "curso",
      accion: "UPDATE_CURSO",
      id_objeto: participanteCursoFormacionVerificado?.voceros?.id
        ? participanteCursoFormacionVerificado?.voceros?.id
        : 0,
      id_usuario: validaciones.id_usuario,
    ...
    return generarRespuesta(
      ...
      {
        participantes: participanteCursoFormacionVerificado,
      },
      201,
    );
  } catch (error) {
