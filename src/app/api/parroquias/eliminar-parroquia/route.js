/**
 @fileoverview Controlador de API para eliminar (o marcar como eliminada) a una parroquia del
 ...
 @module api/parroquias/eliminarParroquia
*/

import prisma from "@/libs/prisma"; // Cliente Prisma para interactuar con la base de datos
import { generarRespuesta } from "@/utils/respuestasAlFront"; // Utilidad para generar respuestas HTTP estandarizadas
import validarEliminarParroquia from "@/services/parroquias/validarEliminarParroquia"; // Servicio para validar la eliminación de parroquia
import registrarEventoSeguro from "@/libs/trigget"; // Servicio para registrar eventos de auditoría

/**
 Maneja las solicitudes HTTP PATCH para eliminar (lógicamente) a una parroquia.
 Valida los datos recibidos, actualiza el campo `borrado` en la base de datos
 y retorna una respuesta estructurada con el perfil actualizado de l parroquia.
 
 @async
 @function PATCH
 @param {Request} request - Solicitud HTTP con el estado de eliminación y el ID de la parroquia.
...
export async function PATCH(request) {
  try {
    ...
    const validaciones = await validarEliminarParroquia(estado, id_parroquia);
    ...
    if (validaciones.status === "error") {
      await registrarEventoSeguro(request, {
        ...
        descripcion: "Validacion fallida al eliminar la parroquia",
        datosAntes: null,
    ...
    // 4. Ejecuta transacción: actualiza el estado de eliminación y consulta el parroquia actualizada
    const [eliminandoParroquia, parroquiaActualizada] =
    ...
    if (!eliminandoParroquia || !parroquiaActualizada) {
      await registrarEventoSeguro(request, {
        tabla: "parroquia",
    ...
    await registrarEventoSeguro(request, {
      tabla: "parroquia",
      accion: "DELETE_PARROQUIA",
      id_objeto: parroquiaActualizada.id,
      id_usuario: validaciones.id_usuario,
      descripcion: "Parroquia eliminada con exito",
      datosAntes: null,
      datosDespues: {
        eliminandoParroquia,
        parroquiaActualizada,
    ...
    return generarRespuesta(
      "ok",
      "parroquia eliminado correctamente...",
      {
    ...
    console.log(`Error interno (eliminar parroquia): ` + error);
    ...
    await registrarEventoSeguro(request, {
      ...
      id_objeto: 0,
      id_parroquia: 0,
      descripcion: "Error inesperado al eliminar parroquia",
      datosAntes: null,
    ...
    return generarRespuesta(
      "error",
      "Error, interno (eliminar parroquia)",
      {},
      500
    );
  }
