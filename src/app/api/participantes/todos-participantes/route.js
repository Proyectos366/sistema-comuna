/**
  @fileoverview Controlador de API para la consulta de todos los participantes. Este archivo maneja la
  lógica para obtener todos los participantes de la base de datosa través de una solicitud GET. Utiliza
...
import validarConsultarTodosParticipantes from "@/services/participantes/validarConsultarParticipantes";

/**
  Maneja las solicitudes HTTP GET para obtener todos los participantes.
  @async@function GET
...
export async function GET() {
  try {
    ...
    const validaciones = await validarConsultarTodosParticipantes();
    ...
    const todosParticipantes = await prisma.curso.findMany({
      where: { borrado: false },
      include: {
        voceros: {
          select: {
    ...
    console.log(`Error interno consultar (participantes): ` + error);
