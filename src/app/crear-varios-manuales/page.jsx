...
export default function CrearManuales() {
  ...
  const crearRoles = async () => {
   ...
   const crearParroquias = async () => {
    try {
      const responseParroquias = await axios.get(`/api/crear-manuales/crear-parroquias`);
  ...
  const crearComunasVillaDeCura = async () => {
    try {
      const responseComunasVillaDeCura = await axios.get(`/api/crear-manuales/crear-comunas`);
    ...
    <div className="grid min-h-dvh grid-rows-[auto_1fr_auto] space-y-3 container mx-auto">
      ...
      <main className="border bg-white flex items-center justify-center">
        <section className="flex space-x-10">
          <div>
            <button className="cursor-pointer borde-fondo px-6 py-2 rounded-md bg-red-300 hover:bg-red-400 hover:px-8" onClick={() => crearRoles()}>
              Crear roles
            </button>
          </div>

          <div>
            <button className="cursor-pointer borde-fondo px-6 py-2 rounded-md bg-red-300 hover:bg-red-400 hover:px-8" onClick={() => crearParroquias()}>
              Crear parroquias
            </button>
          </div>

          <div>
            <button className="cursor-pointer borde-fondo px-6 py-2 rounded-md bg-red-300 hover:bg-red-400 hover:px-8" onClick={() => crearComunasVillaDeCura()}>
              Crear comunas
            </button>
          </div>
        </section>
