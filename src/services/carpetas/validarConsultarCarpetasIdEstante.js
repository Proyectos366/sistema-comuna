/**
 @fileoverview Función utilitaria para validar la identidad del usuario y un ID de estante antes
 de consultar las carpetas.
 @module services/carpetas/validarConsultarCarpetasIdEstante
*/
...
/**
 Valida la identidad del usuario y el ID del estante para consultar carpetas.
 @async
 @function validarConsultarCarpetasIdEstante
...
*/
export default async function validarConsultarCarpetasIdEstante(request) {
  try {
    // 1. Extraer el parámetro 'idEstante' de la URL de la solicitud.
    const { searchParams } = new URL(request.url);
    const idEstante = searchParams.get("idEstante");
    ...
    // 4. Validar que el 'idEstante' sea un ID válido.
    const validarIdEstante = ValidarCampos.validarCampoId(idEstante, "estante");

    // 5. Si la validación del ID del estante falla, se retorna un error.
    ...
    console.log(
      `Error interno validar consultar carpetas id estante: ` + error,
    );

    // Retorna una respuesta del error inesperado
    return retornarRespuestaFunciones(
      "error",
      "Error interno validar consultar carpetas id estante",
    );
  }
}
