export default function InputFile({ id, onChange, value }) {
  return (
    <div className={`flex space-x-10 sm:w-1/3 space-y-1 h-20 border`}>
          ...
          >
            ...
            <input
              id="file-upload"
              type="file"
              key={archivo ? archivo.name : "empty"} // Cambia dinámicamente la clave
              onChange={handleFileChange}
              className="hidden"
              accept="application/pdf"
            />
          </label>
          <div className={`${archivo ? "flex" : "hidden"}`}>
            <img
              className="w-16 h-full"
              src="/img/pdf-subir-archivo.jpg"
              alt=""
            />
          </div>
        </div>
