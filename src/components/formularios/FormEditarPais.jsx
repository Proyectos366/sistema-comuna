...
export default function FormEditarPais({
  ...
  useEffect(() => {
    const validarYActualizar = (valor, setValor, setValidar) => {
      if (valor) {
        const limpio = String(valor).trim();
        const esValido = /^[a-zA-Z\sñÑáéíóúÁÉÍÓÚ]+$/.test(limpio);

        if (typeof setValidar === "function") setValidar(esValido);
        if (typeof setValor === "function") setValor(limpio);
      }
    };

    validarYActualizar(nombre, setNombre, setValidarNombre);
    validarYActualizar(capital, setCapital, setValidarCapital);
  }, [nombre, capital]);
