...
export default function ListadoNovedades({ novedad, editarNovedad }) {
  ...
  return (
    <Div className="bg-white py-2 px-2 sm:px-4 text-sm sm:text-md flex flex-col gap-1 text-black rounded-b-md">
      ...
      <Div className="flex items-center justify-between">
        <BloqueInfo
          indice={!novedad.borrado ? 3 : 2}
          nombre={"novedad"}
          valor={!novedad.borrado ? "Activo" : "Inactivo"}
        />

        <SwitchToggle
          checked={!novedad.borrado}
          onToggle={() => {
            dispatch(
              eliminarRestaurarNovedad({
                estado: novedad.borrado,
      ...
      <BloqueInfo
        indice={1}
        nombre={"Creada"}
        valor={formatearFecha(novedad.createdAt)}
      />
    </Div>
  );
}
